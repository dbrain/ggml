...
// Backend-level Q tiling for large unmasked cuDNN SDPA. Unlike graph-level
...
// GGML_CUDNN_LTX_QTILE remains an alias for existing LTX deployments.
static bool cudnn_qtile(ggml_backend_cuda_context & ctx, ggml_tensor * dst,
    ...
    const char * e = getenv("GGML_CUDNN_ATTN_QTILE");
    if (e == nullptr) {
        e = getenv("GGML_CUDNN_LTX_QTILE");
    }
    ...
    static int traced = 0;
    if (traced++ < 4) {
        fprintf(stderr, "[cudnn-qtile] Lq=%lld tile=%lld H=%lld\n",
