...
struct CollectiveMainloopFwd {
    ...
    int get_n_block_max(Params const& mainloop_params, int m_block) {
        ...
        if constexpr (Is_causal) {
            const int causal_max = cute::ceil_div((m_block + 1) * kBlockM + seqlen_k - seqlen_q, kBlockN);
            n_block_max = n_block_max < causal_max ? n_block_max : causal_max;
        }
        return n_block_max;
    }
        ...
        auto add_delta_s = [&](auto& acc) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if constexpr (std::is_same_v<ElementDS, float>) {
                    ...
                    auto tSsDS_stage = recast<cutlass::Array<ElementDS, 8>>(sDS(_, _, smem_pipe_read_k.index()));
                    // A 16-byte half packet contains two adjacent logical
                    // float4 correction vectors. num is always even here.
                    const auto ds0 = tSsDS_stage(make_coord(_0{}, _0{}), make_coord(num / 2, _0{}));
                    const auto ds1 = tSsDS_stage(make_coord(_0{}, _0{}), make_coord((num + 1) / 2, _0{}));
                    delta_s_0 = make_float4(float(ds0[0]), float(ds0[1]), float(ds0[2]), float(ds0[3]));
                    delta_s_1 = make_float4(float(ds1[4]), float(ds1[5]), float(ds1[6]), float(ds1[7]));
                }
