...
#pragma unroll
        for (int i0 = 0; i0 < I; i0 += warp_size) {
            ...
            // The NVFP4 per-channel activation scale is folded into the GEMM epilogue, so it must
            // be applied to `val` BEFORE the accumulate branch -- `dst += y_scale*val`, never
            // `y_scale*(dst + val)`.
...
#pragma unroll
            for (int l = 0; l < tile_C::ne; ++l) {
                ...
                // Scale before accumulating -- see the note in the dp4a write_back above.
                if constexpr (type == GGML_TYPE_NVFP4) {
                    if (y_scale_used) {
                        val *= y_scale[j];
                    }
