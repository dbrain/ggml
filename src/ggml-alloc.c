    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            ...
            if (p_hn->n_children == 0 && p_hn->n_views == 0) {
                if (ggml_impl_is_view(parent)) {
                    // A view that is ITSELF a graph output aliases its view_src's
                    // buffer and must persist for the whole graph. Releasing the
                    // view_src (free_node only checks the view_src's own OUTPUT
                    // flag, not its views') would reuse that buffer and clobber the
                    // output view. So keep the view_src pinned: skip the n_views
                    // decrement + free. (free_node already protects the view itself,
                    // but the view owns no buffer — its storage is the view_src.)
